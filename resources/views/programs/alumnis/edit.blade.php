            ...
            <ol class="inline-flex items-center space-x-3">
                ...
                <li>
                    <div class="flex items-center gap-1">
                        ...
                        <a href="{{ route('programs.show', $program->id) }}" class="ms-1 text-sm font-medium text-gray-500 hover:text-gray-600">Program Alumni</a>
                    </div>
                </li>
        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="bg-white overflow-hidden border border-gray-200 md:rounded-2xl">
                <div class="p-6 text-gray-900 space-y-5">
                    <form method="POST" action="{{ route('programalumnis.update', $alumni->id) }}" enctype="multipart/form-data" class="space-y-4">
                        @csrf
                        @method('PATCH')
                        <div class="grid gap-5 mb-4 grid-cols-1 md:grid-cols-3">
                            ...
                            <div>
                                ...
                                <input type="file" name="photo" id="photo" value="{{ $alumni->photo }}"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" />
                                <p class="mt-1 text-xs text-gray-500">
                                    <span class="text-red-500 text-xs">{{ $errors->first('photo') }}</span>
                                </p>
                            </div>
                            <div>
                            ...
                            <div>
                                <label for="work" class="block mb-2 text-sm font-medium text-gray-900">Tempat
                                    Bekerja</label>
                                <input type="text" name="work" id="work" value="{{ $alumni->work }}"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                    placeholder="Tempat Bekerja" required />
                                <p class="mt-1 text-xs text-gray-500">
                                    <span class="text-red-500 text-xs">{{ $errors->first('work') }}</span>
                                </p>
                            </div>
                            <div>
                                <label for="profession"
                                    class="block mb-2 text-sm font-medium text-gray-900">Profesi</label>
                                <input type="text" name="profession" id="profession" value="{{ $alumni->profession }}"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                    placeholder="Profesi" required />
                                <p class="mt-1 text-xs text-gray-500">
                                    <span class="text-red-500 text-xs">{{ $errors->first('profession') }}</span>
                                </p>
                            </div>
                        </div>
                        <div>
                            ...
                            <textarea name="quote" id="quote" value="{{ $alumni->quote }}"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                placeholder="Quote" required>{{ $alumni->quote }}</textarea>
                            <p class="mt-1 text-xs text-gray-500">
                                <span class="text-red-500 text-xs">{{ $errors->first('quote') }}</span>
                            </p>
                        </div>
                        <button type="submit"
