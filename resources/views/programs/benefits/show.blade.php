    ...
    <div class="py-5 space-y-10">
        ...
        <section class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
            <div class="flex flex-col md:flex-row items-center justify-between md:items-end gap-4 md:gap-0">
                <a href="{{ route('programbenefits.create', $program->id) }}"
                                ...
                                @forelse ($benefits as $no => $benefit)
                                    <tr class="odd:bg-white even:bg-gray-50 border-b">
                                        ...
                                        <td class="px-6 py-4 flex gap-2">
                                            <form action="{{ route('programbenefits.updatestatus', $benefit->id) }}"
                                                method="POST" onclick="return confirmUpdate();">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit"
                                                    class="{{ $benefit->status ? 'bg-emerald-500 hover:bg-emerald-600' : 'bg-red-500 hover:bg-red-600' }} px-4 py-1.5 text-sm font-medium rounded-xl text-white transition-all ease-in-out">
                                                    {!! $benefit->status ? '<i class="fa-solid fa-toggle-on"></i>' : '<i class="fa-solid fa-toggle-off"></i>' !!}
                                                </button>
                                            </form>
                                            <a href="{{ route('programbenefits.edit', $benefit->id) }}"
                                                class="bg-amber-500 hover:bg-amber-600 text-white px-4 py-2 text-xs rounded-xl hover font-medium">
                                                <i class="fa-regular fa-pen-to-square"></i>
                                            </a>
                                            <form
                                                action="{{ route('programbenefits.destroy', ['id' => $benefit->id, 'program_id' => $benefit->program_id]) }}"
